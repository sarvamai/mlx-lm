...
class SarvamMoEAttention(nn.Module):
    ...
    def __call__(
        ...
        # Apply RoPE
        # Use mx.fast.rope
        # Matches previous behavior: Full head_dim was used for RoPE
...
class SarvamMoEDecoderLayer(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        r = self.attention(self.input_layernorm(x), mask, cache, offset)
        h = x + r
...
class SarvamMoEModel(nn.Module):
    ...
    def __call__(
        ...
        # position_ids removed, using offset passed to layers
        start = 0
